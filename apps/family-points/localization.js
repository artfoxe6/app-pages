const translations = {
  zh: {
    ...
    privacy: {
      ...
      storageText: "这些数据仅用于提供您选择的功能，例如计算积分、展示任务和成长记录、生成本地提醒、保护 App 访问，以及在您开启时创建 iCloud 备份。数据默认保存在您的设备上。开启 iCloud 自动备份后，家人昵称及资料标识、您选择的照片、任务与积分等家庭内容、操作记录和设备名称会传输并保存到您 Apple 账号的 iCloud Drive 中，并受 Apple 服务条款和您的 iCloud 设置约束。",
  ...
  ja: {
    ...
    privacy: {
      ...
      storageText: "これらのデータは、ポイント計算、タスクや成長記録の表示、端末内通知、アプリへのアクセス保護、お客様が有効にしたiCloudバックアップなど、選択された機能の提供にのみ使用されます。データは初期状態では端末に保存されます。iCloud自動バックアップを有効にすると、家族のニックネームとプロフィール識別情報、選択した写真、タスクやポイントなどの家族コンテンツ、操作履歴、デバイス名が、お客様のApple AccountのiCloud Driveへ転送・保存され、Appleの利用規約とiCloud設定が適用されます。",
  ...
  ko: {
    ...
    privacy: {
      ...
      storageText: "이 데이터는 포인트 계산, 할 일과 성장 기록 표시, 기기 내 알림 생성, 앱 접근 보호 및 사용자가 활성화한 iCloud 백업 등 선택한 기능을 제공하는 데만 사용됩니다. 데이터는 기본적으로 사용자의 기기에 저장됩니다. iCloud 자동 백업을 켜면 가족 별명과 프로필 식별 정보, 선택한 사진, 할 일과 포인트 등의 가족 콘텐츠, 작업 기록 및 기기 이름이 사용자의 Apple 계정에 연결된 iCloud Drive로 전송되어 저장되며 Apple 서비스 약관과 iCloud 설정의 적용을 받습니다.",
  ...
  en: {
    ...
    privacy: {
      ...
      storageText: "This data is used only to provide features you choose, such as calculating points, showing tasks and growth records, creating local reminders, protecting access to the App, and creating iCloud backups when enabled. Data is stored on your device by default. When automatic iCloud backup is enabled, family nicknames and profile identifiers, selected photos, family content such as tasks and points, activity records, and the device name are transferred to and stored in iCloud Drive under your Apple Account, subject to Apple's terms and your iCloud settings.",
